package cn.revoist.lifephoton.module.funga.core.genephenotype.service
...
import cn.revoist.lifephoton.module.funga.core.genephenotype.model.PredictGene
import cn.revoist.lifephoton.module.funga.core.genephenotype.model.SearchContainer
import cn.revoist.lifephoton.module.funga.core.genegene.model.GeneInteractionMapper
import cn.revoist.lifephoton.module.funga.core.common.model.GeneListWithDatabaseRequest
import cn.revoist.lifephoton.module.funga.core.genephenotype.model.FuncGeneResponse
import cn.revoist.lifephoton.module.funga.core.genephenotype.model.GeneResult
import cn.revoist.lifephoton.module.funga.core.genephenotype.model.ImputationFunGenesRequest
import cn.revoist.lifephoton.module.funga.core.genephenotype.model.ImputationPredictGenesRequest
import cn.revoist.lifephoton.module.funga.core.genephenotype.model.ImputationResultRequest
import cn.revoist.lifephoton.module.funga.core.genephenotype.model.PhenotypeReferences
...
object GenePhenotypeService {
    ...
    fun nohupImputationFunGenes(request: ImputationFunGenesRequest, user: UserDataEntity): String {
        ...
        submit(-1, -1) {
            try {
                ...
                val funcGenes = imputationFunGenes(user.id,request)
                ...
                val g3 = predictGenes.filter { it.data["genes"] != null }
                    .map { (it.data["genes"] as HashMap<String, Int>).keys }.flatten()
                req.genes = g1 + g2 + g3
                ...
                val data = hashMapOf(
                    ...
                    "graph" to GeneGeneService.getInteractionsByGeneList(req),
                )
                ...
                FungaPlugin.dataManager.useDatabase()
                    .insert(GenePhenotypeAnalysisTable) {
                        ...
                        set(
                            GenePhenotypeAnalysisTable.result,
                            gson.fromJson(gson.toJson(data), AnalysisResult::class.java)
                        )
    ...
    fun imputationFunGenes(user_id:Long,request: ImputationFunGenesRequest): List<MergeData<List<FuncGeneResponse>>> {
            ...
            phenotypes.forEach { gene, ps ->
                ...
                try {
                    val isRelated = if (request.type == "union") {
                        ppp = AnalysisAssistant.create(user_id).isPhenotypeRelatedAny(request.phenotypes, ps, KnowledgeBaseAggregator.options(user_id))
                        ppp.result.isNotEmpty()
                    } else {
                        ppp = AnalysisAssistant.create(user_id).isPhenotypeRelatedAll(request.phenotypes, ps, KnowledgeBaseAggregator.options(user_id))
                        ppp.result.isNotEmpty()
                    }
                    if (isRelated) {
                        val result = FungaPlugin.dataManager.useDatabase(db)
                            .mapsWithColumn(
                                GenePhenotypeTable,
                                GenePhenotypeTable.phenotype,
                                GenePhenotypeTable.references
                            ) {
                                where {
                                ...
                                PhenotypeReferences(
                                    ...
                                    (it["references"] as List<String>).distinct(),
                                    ppp.result[it["phenotype"].toString()].toString()
                                )
                            }
    ...
    fun imputationPredictGene(request: ImputationPredictGenesRequest, currentDegree:Int = 1): List<MergeData<HashMap<String, Any>>> {
        ...
        return join(request.dbs()) { db ->
            ...
            GeneGeneService.getInteractionsByPGR(filterRes, db)
        }
    }
                ...
                GenePhenotypeTable.search(db, hashMapOf("phenotype" to it), request.topK).forEach {
                    if (!ids.contains(it["gene"])) {
                        ...
                        result[it["gene"]]?.add(
                            PhenotypeReferences(
                                it["phenotype"].toString(),
                                (it["references"] as JsonArray).map { it.asString }.distinct(),
                                "causality"
                            )
                        )
                    }
        ...
        return when(request.type){
            "discussion" -> {
                if (request.genes.isEmpty()){
                    ...
                    AnalysisAssistant.create(user.id).generateDiscretion(
                        gson.toJson(hashMapOf(
                        "func" to func,
                        "outer" to outer,
                        "network" to network,
                    )),request.prompt, KnowledgeBaseAggregator.options(user.id))
                }
            }
            "graph" -> {
    ...
    fun funcGeneSort(user_id: Long,geneData: Map<String, List<String>>,phenotype: String,db: String): SortedGeneResult {
        return AnalysisAssistant.create(user_id).sort(geneData,phenotype,db, KnowledgeBaseAggregator.options(user_id))
    }
}